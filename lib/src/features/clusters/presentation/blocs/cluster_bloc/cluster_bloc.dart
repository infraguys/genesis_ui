...
import 'package:genesis/src/features/clusters/domain/usecases/create_cluster_usecase.dart';
import 'package:genesis/src/features/clusters/domain/usecases/delete_cluster_usecase.dart';
import 'package:genesis/src/features/clusters/domain/usecases/get_cluster_usecase.dart';
import 'package:genesis/src/features/clusters/domain/usecases/update_cluster_usecase.dart';
...
class ClusterBloc extends Bloc<ClusterEvent, ClusterState> with PollingBlocMixin {
  ...
  Future<void> _onGetCluster(_Get event, Emitter<ClusterState> emit) async {
    ...
    final cluster = await useCase(event.id);
  ...
  Future<void> _onDeleteCluster(_Delete event, Emitter<ClusterState> emit) async {
    ...
    try {
      await useCase(event.cluster.id);
