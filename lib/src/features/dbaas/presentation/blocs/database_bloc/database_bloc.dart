...
import 'package:genesis/src/features/dbaas/domain/use_cases/database_usecases/get_database_usecase.dart';
...
class DatabaseBloc extends Bloc<DatabaseEvent, DatabaseState> {
  ...
  Future<void> _onGetDatabase(_GetDatabase event, Emitter<DatabaseState> emit) async {
    final useCase = GetDatabaseUseCase(_repository);
    emit(DatabaseLoadingState());
    final database = await useCase(event.params);
    emit(DatabaseLoadedState(database));
  }
  ...
  // Future<void> _onUpdateDatabase(_UpdateDatabase event, Emitter<DatabaseState> emit) async {
  //   final useCase = UpdateD(_repository);
  //   final database = await useCase(event.params);
